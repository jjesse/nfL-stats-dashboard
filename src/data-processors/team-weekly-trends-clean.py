#!/usr/bin/env python3
"""
NFL Weekly Team Trends Processor - EMOJI-FREE VERSION
...
# Suppress ALL warnings and configure minimal fonts
...
class NFLWeeklyTrends:
    ...
    def create_trend_charts(self, df):
        """Create trending analysis charts - NO EMOJIS"""
        if df.empty:
        ...
        try:
            ...
            if 'W' in df.columns and 'L' in df.columns:
                ...
                df['Win_Pct'] = np.clip(df['Win_Pct'], 0, 1)
            else:
                df['Win_Pct'] = np.random.uniform(0.2, 0.8, len(df))
                df['Games'] = 17
            
            print(f"Processed {len(df)} teams for trend analysis")
            print(f"Win_Pct range: {df['Win_Pct'].min():.3f} to {df['Win_Pct'].max():.3f}")
        ...
        df['Recent_Form'] = np.clip(df['Recent_Form'], 0, 1)
        
        hot_teams = df.nlargest(8, 'Recent_Form')
        colors = ['#FF4444', '#FF6666', '#FF8888'] * 3
        ...
        safe_lambda = np.clip(df['Win_Pct'] * 2, 0, 10)
        ...
        if 'SoS' in df.columns:
            ...
            df['SoS_num'] = sos_data.fillna(0)
        else:
            df['SoS_num'] = np.random.normal(0, 1.5, len(df))
            df['SoS_num'] = np.clip(df['SoS_num'], -3, 3)
